...
We prove $A_n \text{ char } S_n$ for all $n \in \Z^+$ by showing subgroup in index 2 is unique. For all $|S_n : H| = 2$, we have $H \unlhd S_n$. Thus there exists onto $\phi : S_n \to Z_2$ that $\ker \phi = H$ by $S_n \to S_n / H \to Z_2$. If there is a transposition $p$ that $\phi(p) = 1$, then all transposition need to be in kernel, thus $\ker \phi = S_n \ne H$. So all transposition not in kernel, thus we can see $\phi(\sigma) = \text{sgn}(\sigma)$. So $H = A_n$.
...
a. Consider $\phi : G_I \to G_{I_1} \times G_{I_2} \times \cdots \times G_{I_m}$ that $\phi(a_1, a_2, \cdots, a_n) = (a_{I_1}, a_{I_2}, \cdots, a_{I_m})$. Easy to see $\phi^{-1}(a_{I_1}, a_{I_2}, \cdots, a_{I_m}) = (a_1, a_2, \cdots, a_n)$, where $a_i = 1$ if $i \notin I$. And easy to verify that $\phi$ is iso.
...
Since there is $\phi_j : G \to G_J$ that $\phi_j(a_1, \cdots, a_n) = (a'_1, \cdots, a'_n)$ that $a'_i = a_i$ if $i \in J$, $a'_i = 1$ if $i \notin J$, and obviously $a \in \phi_j^{-1}(a)$, $\ker \phi_j = G_I$, then $G/G_I \cong G_J$.

c. Consider $\phi' : G \to G_I \times G_J$ that $\phi'(a) = (\phi_i(a), \phi_j(a))$. Obviously $\phi'^{-1}(a, b) = (c_1, \cdots, c_n)$ that $c_i = a_i$ if $i \in I$, $c_i = b_i$ if $i \notin I$., and easy to verify $\phi'$ is iso.

## P156 T7

Let $G_1, G_2, \cdots, G_n$ be groups and let $\pi$ be a fixed element of $S_n$. Prove that the map $\phi_\pi : G_1 \times \cdots \times G_n \to G_{\pi^{-1}(1)} \times \cdots \times G_{\pi^{-1}(n)}$ defined by $\phi_\pi(g_1, \cdots, g_n) = (g_{\pi^{-1}(1)}, \cdots, g_{\pi^{-1}(n)})$ is an isomorphism (so that changing the order of the factors in a direct product does not change the isomorphism type).
...
b. Let $Z_4 = \langle x \rangle$. Let $D_8 = \langle r, s \rangle$ and $Q_8 = \langle i, j \rangle$ be given by their usual generators and relations. Let $Z_4 * D_8$ be the central product of $Z_4$ and $D_8$ which identifies $x^2$ and $r^2$ (i.e., $Z_1 = \langle x^2 \rangle$, $Z_2 = \langle r^2 \rangle$ and the isomorphism is $x^2 \to r^2$) and let $Z_4 * Q_8$ be the central product of $Z_4$ and $Q_8$ which identifies $x^2$ and $-1$. Prove that $Z_4 * D_8 \cong Z_4 * Q_8$.

---

a. We first prove $A \to A':=\{(a, 1)Z \mid a \in A\}$ is iso. We have $(a, 1)Z = (a', 1)Z$ iff $(a^{-1}a', 1) \in Z$ iff $a^{-1}a' = 1$ iff $a=a'$. So easy to see $\phi(a) = (a, 1)Z$ is a iso.

Likewise, $B \to B':=\{(1, b)Z \mid b \in B\}$ is iso.

We have $(a, 1)Z = (1, b)Z$ iff $(a, b^{-1}) \in Z$ iff $a \in Z_1, a \to b$. So $A' \cap B' = Z':=\{(a, b^{-1})Z \mid a \in Z_1, a \to b\}$. Easy to see $\phi(a) = (a, b^{-1})Z$ where $a \to b$ is a iso. from $Z_1$ to $Z'$.

For all $(g, h)Z \in A*B$, $a \in Z_1$, $a \to b$, we have $[(g, h)Z][(a, b^{-1})Z][(g, h)Z]^{-1} = (gag^{-1}, hb^{-1}h^{-1})Z = (a, b^{-1})Z$ since $a \in Z_1$, $b \in Z_2$. So $Z' \le Z(A*B)$.

Obviously $|A*B|=\frac{|A \times B|}{|Z|} = \frac{|A| \cdot |B|}{|Z_1|}$.

b. Let $Z_1 = \langle (x^2, r^2) \rangle$, $Z_2 = \langle (x^2, i^2) \rangle$. Consider $\phi : Z_4 * D_8 \to Z_4 * Q_8$ that $\phi((x^a, r^bs^c)Z_1) = (x^{a-c}, i^bj^c)Z_2$.
...
Consider onto $\phi_i:A_i \to A_i/B_i$ that $\phi_i(a) = aB_i$. Consider $\phi:G \to (A_1/B_1) \times \cdots \times (A_n/B_n)$ that $\phi(a_1, \cdots, a_n) = (\phi_1(a_1), \cdots, \phi_n(a_n))$. Obviously $\phi$ is onto, and $\ker \phi = \ker \phi_1 \times \cdots \times \ker \phi_n = B_1 \times \cdots \times B_n$. This complete the proof.
...
Pick a fixed $g \in G$. We have $gxg^{-1} \in \langle x \rangle$, so $gxg^{-1} = x^a$. Let $\phi_g : H \to H$ that $\phi_g(y) = gyg^{-1}$, if $y = x^b$, then $\phi_g(y) = \phi_g(x^b) = \phi_g(x)^b = (x^a)^b = x^{ab} = (x^b)^a = y^a$. So $\phi_g(y) = y^a$. Then $\phi_{g^{p-1}}(y) = \phi_g^{p-1}(y) = y^{a^{p-1}} = y$ by Fermat's Little Theorem.

Consider $\sigma : G \to G^{p-1} := \{g^{p-1} \mid g \in G\}$ that $\sigma(g) = g^{p-1}$. Obviously $\sigma$ is onto. Suppose that $g^{p-1} = h^{p-1}$, we also have $g^{|G|} = 1 = h^{|G|}$. Since $\gcd(p - 1, |G|) = 1$, we have $s(p-1) + t|G| = 1$, so $g = (g^{p-1})^s(g^{|G|})^t = (h^{p-1})^s(h^{|G|})^t = h$. So $\sigma$ is one-to-one, thus $\sigma$ is bijection. Obviously $G^{p-1} \subseteq G$, then $G^{p-1} = G$.
...
Since $H \unlhd G$, consider $G$ act on $H$ conjugately. Let $\sigma_g : H \to H$ that $\sigma_g(y) = g \cdot y$. Consider $\phi : G \to \text{Aut}(H)$ that $\phi(g) = \sigma_g$ is a homo. and $\ker \phi = \{g \in G \mid \sigma_g = I\} = \{g \in G \mid gyg^{-1} = y \space \forall y \in H\} = C_G(H)$.
